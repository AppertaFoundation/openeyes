...
class Element_OphCoCvi_ClericalInfo extends \BaseEventTypeElement
{
    ...
    protected function afterSave()
    {
        if (!empty($_POST['ophcocvi_clinicinfo_patient_factor_id'])) {
            $existing_ids = array();
            foreach (OphCoCvi_ClericalInfo_PatientFactor_Answer::model()->findAll('element_id = :elementId',
            ...
            foreach ($existing_ids as $id) {
                if (!in_array($id, $_POST['ophcocvi_clinicinfo_patient_factor_id'])) {
                    $item = OphCoCvi_ClericalInfo_PatientFactor_Answer::model()->find('element_id = :elementId and ophcocvi_clinicinfo_patient_factor_id = :lookupfieldId',
    ...
    {
        $data = array();
        $employement_status = (OphCoCvi_ClericalInfo_EmploymentStatus::model()->findAll('`active` = ?', array(1),
    ...
    {
        $data = array();
        $preferredInfoFormats = (OphCoCvi_ClericalInfo_PreferredInfoFmt::model()->findAll('`require_email` = ?',
            array(0), array('order' => 'display_order asc')));
        ...
        $preferredInfoFormatEmail = (OphCoCvi_ClericalInfo_PreferredInfoFmt::model()->findAll('`require_email` = ?',
            array(1), array('order' => 'display_order asc')));
        $emailData = array();
