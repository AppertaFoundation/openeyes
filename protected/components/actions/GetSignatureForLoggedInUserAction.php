...
class GetSignatureForLoggedInUserAction extends \CAction
{
    use RenderJsonTrait;
    ...
    {
        $poseted_user_id = (int)\Yii::app()->request->getPost("user_id");
        $this->user = User::model()->findByPk(\Yii::app()->user->id);

        if ($poseted_user_id !== (int)$this->user->id) {
            $this->renderJSON([
                'code' => 1,
                'error' => "An error occurred while trying to fetch your signature. Please contact support.",
            ]);
        }
    }
