...
<h3 class="withEventIcon"
	style="background: transparent url(/img/_elements/icons/event/medium/treatment_laser.png) center left no-repeat;">
	<?php if (empty($model->event_id)) { ?>Book	Operation<?php } else { ?>Edit Operation<?php } ?></h3>
...
<div id="editEyeSelection" class="eventDetail">
	...
	<div class="data">
		...
		<span class="group">
			<input id="ElementDiagnosis_eye_0" value="1"
			<?php if ($diagnosis->eye == '1') { ?>checked="checked"<?php } ?>
			type="radio" name="ElementDiagnosis[eye]" />
			<label for="ElementDiagnosis_eye_0">Right</label>
		</span>
		<span class="group">
			<input id="ElementDiagnosis_eye_2" value="2"
			<?php if ($diagnosis->eye == '2') { ?>checked="checked"<?php } ?>
			type="radio" name="ElementDiagnosis[eye]" />
			<label for="ElementDiagnosis_eye_2">Both</label>
		</span>
		<span class="group">
			<input id="ElementDiagnosis_eye_1" value="0"
			<?php if ($diagnosis->eye == '0') { ?>checked="checked"<?php } ?>
			type="radio" name="ElementDiagnosis[eye]" />
...
<script type="text/javascript">
	$('input[name="ElementDiagnosis[eye]"]').click(function() {
		...
		// Change operation eye to match
		var diagnosis_eye_id = $(this).val();
		$('input[name="ElementOperation[eye]"][value="'+diagnosis_eye_id+'"]').attr('checked', true);
		
		// Focus disorder input if not completed
