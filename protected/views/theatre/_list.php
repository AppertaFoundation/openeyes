...
?>
<h3 class="sessionDetails"><span class="date"><strong><?php echo date('d M',$timestamp)?></strong> <?php echo date('Y',$timestamp)?></span> - <strong><span class="day"><?php echo date('l',$timestamp)?></span>, <span class="time"><?php echo substr($session['startTime'], 0, 5)?> - <?php echo substr($session['endTime'], 0, 5)?></span></strong> for <?php echo !empty($session['firm_name']) ? $session['firm_name'] : 'Emergency List' ?> <?php echo !empty($session['specialty_name']) ? 'for (' . $session['specialty_name'] . ')' : '' ?> - <?php echo $name?></h3>
...
?>
							<tr id="oprow_<?php echo $session['operationId'] ?>">
								...
								<td class="hospital"><?php echo CHtml::link(
									...
									'/patient/episodes/' . $session['patientHash'] . '/event/' . $session['eventId']
								);
								?></td>
