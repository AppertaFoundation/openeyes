...
<?php
$disableLink = array();
foreach(Yii::app()->params['menu_disable_url'] as $menuKey => $menuValue)
{
	if(Yii::app()->params[$menuKey] === false)
	{
		$disableLink[] = $menuValue;
	}
}
foreach (Yii::app()->params['admin_structure'] as $adminBoxTitle => $adminBoxElements) {
	?>
	<div class="box admin">
		...
		<div class="box_admin_elements">
			<ul class="navigation admin">
				<?php
				foreach ($adminBoxElements as $title => $uri) {
					...
					if ($uri && !in_array($uri,$disableLink)) { ?>
						<li<?php
