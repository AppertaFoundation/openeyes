...
<div class="cols-7">
    ...
    <table class="standard" id="institution-authentications">
        ...
        <?php foreach ($institution_authentications as $key => $institution_authentication) { ?>
            <tr class="clickable"
                data-id="<?= $institution_authentication->id ?>"
                data-uri="admin/editinstitutionauthentication?institution_authentication_id=<?= $institution_authentication->id ?>"
                data-key="<?=$key?>"
            >
...
<div class="cols-7">
    ...
    <hr class="divider">
    <?php $patient_identifier_type_fields = ['id', 'site_id', 'usage_type', 'short_title', 'long_title',
...
<div class="cols-7">
    ...
    <hr class="divider">
    <?= \CHtml::dropDownList(
        'patient_identifier_usage_type', $patient_identifier_usage_type, CHtml::listData(PatientIdentifierType::model()->findAll(['select' => 't.usage_type', 'distinct' => true]), 'usage_type', 'usage_type')
    ); ?>
    <?= \CHtml::dropDownList(
        'patient_identifier_site', $patient_identifier_site, CHtml::listData(Site::model()->findAll('institution_id=:institution_id', [':institution_id' => $institution->id]), 'id', 'name'), ['empty' => 'NOT SITE SPECIFIC']
    ); ?>
        ...
        <?php if (count($identifier_rules) > 0) {
            foreach ($identifier_rules as $row_count => $identifier_rule) { ?>
                <tr data-row="<?= $row_count ?>"
                    data-id="<?= $identifier_rule->id ?>"
                    data-pid_type="<?= $identifier_rule->patient_identifier_type_id ?>">
                    <td>
                        <?php
                    ...
                    <td><?= $identifier_rule->patientIdentifierType->institution->name ?: '-' ?></td>
...
<script>
    $(document).ready(function () {
        ...
        $('#add-institution-sites-btn').on('click', function () {
            ...
            let tr = Mustache.render($('#add-site-template').text(), {key : nextDataKey});
            $table.append(tr);
        });
        ...
        $('#add-patient-identifier-btn').on('click', function () {
            ...
            let nextDataKey = OpenEyes.Util.getNextDataKey($table.find('tr'), 'key');
            let tr = Mustache.render($('#add-patient-identifier-template').text(), {key : nextDataKey});
            $table.append(tr);
        });
        ...
        new OpenEyes.UI.AdderDialog({
            ...
            itemSets: [new OpenEyes.UI.AdderDialog.ItemSet(<?= CJSON::encode(
                array_map(function ($identifier) {
                    return ['label' => $identifier->long_title . ' (' . $identifier->institution->name . ')',
                        'id' => $identifier->id,
