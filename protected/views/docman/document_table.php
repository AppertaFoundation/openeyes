...
?>
        ...
        if(isset($data["doctargets"])) {
            foreach($data["doctargets"] as $doc_target) {
                ...
                foreach ($data["docoutputs"] as $doc_output) {
                    // TODO: need proper check + data here!!
                    if ($doc_output["document_instance_data_id"] == $doc_target["id"])
                    {
                        $row_count++;
                    }
                }
                            ...
                            <?php if ($line_count == 0) { ?>
                                <td rowspan="<?php echo $row_count; ?>">
                                    ...
                                    <?php if(!isset($data['correspondence_mode']))
                                        {?>
                                            <br>
                                            <a href="/OphCoCo rrespondence/default/update/<?php echo getCorrespondenceId($doc_target["document_instance_id"])?>">Edit Content</a>
                                        <?php
                                        }
                                    ?>
                                </td>
    ...
    if(isset($data["correspondence_mode"]) && $data["correspondence_mode"]){
        $this->renderPartial('/docman/document_row_edit', array('data'=>null));
    ?>
    <?php
    }else{
    ?>
