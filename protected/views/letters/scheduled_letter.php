...
<p>
	...
	if ($site->id == 5) {
		// St George's ?>
	I am pleased to confirm the date of your child's operation with <?php echo $consultantName ?>, the details are:
	<?php } else {
		// City Road ?>
	I am pleased to confirm the date of your child's eye operation, the	details are:
	<?php }} ?>
</p>
...
<p>
	<em>This is a daycase and your child will be discharged from hospital on the same day.</em>
</p>
<?php } ?>
...
<ul>
	...
	<?php if ($site->id == 5) {
		// St Georges ?>
	<li>Please go directly to Duke Elder Ward on level 5 of the Lanesborough wing</li>
	<?php } else { ?>
	<li>Please go directly to the Main Reception on level 5 of the RDCEC</li>
	<?php } ?>
</ul>
...
<ul>
	...
	<li>Please go directly to <?php if ($specialty->id == 13) {
		...
		Floor<?php } else { ?><?php echo CHtml::encode($operation->booking->ward->name) ?> ward<?php } ?></li>
