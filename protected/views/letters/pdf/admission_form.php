...
<h2>Admission Information</h2>
	<table class="borders">
		...
		<tr>
			...
			?>
			<td><?php if($emergencyList) { ?>Emergency List (<?php echo CHtml::encode($consultantName); ?>)<?php } else { echo CHtml::encode($consultantName); 
			}?>
			</td>
			<th>Decision to admit (or today's) date:</th>
			<td><?php echo $operation->NHSDate('decision_date'); ?>
			</td>
		</tr>

		<tr>
			<th>Service:</th>
			<td><?php echo CHtml::encode($firm->serviceSubspecialtyAssignment->service->name) ?>
			</td>
			<th>Patient Telephone:</th>
			<td><?php echo CHtml::encode($patient->primary_phone) ?>
			</td>
		</tr>

		<tr>
			<th>Site:</th>
			<td><?php echo CHtml::encode($site->name) ?>
			</td>
			<?php if($booking) { ?>
			<th>Person organising operation:</th>
			<td><?php echo $booking->user->getFullName() ?>
			</td>
			<?php } else { ?>
			<th>Person organising admission:</th>
			<td><?php echo $operation->event->user->getFullName() ?>
			</td>
			<?php } ?>
		</tr>
...
<table class="borders">
	...
	<tr>
		...
		<td><?php echo ($operation->overnight_stay) ? 'an overnight stay' : 'day case'; ?>
		</td>
	</tr>

	<tr>
		<th>Consultant to be present:</th>
		<td><?php echo (empty($operation->consultant_required)) ? 'No' : 'Yes'; ?>
		</td>
	...
	<tr>
		...
		<?php } else { 	?>
		<th colspan="2" rowspan="4">Patient Added to partial bookings waiting
			List, admission Date to be arranged</th>
		<?php } ?>
	</tr>
	...
	<tr>
		...
		<td><?php echo substr($booking->session->start_time,0,5) . ' - ' . substr($booking->session->end_time,0,5)?>
		</td>
	</tr>
	<tr>
		<th>Theatre:</th>
	...
	<tr>
		<th>Diagnosis:</th>
		<td><?php if ($operation->getDisorder()) {
...
<table class="borders">
	<tr>
		<td height="50"><?php echo nl2br(CHtml::encode($operation->comments)); ?>
		</td>
	</tr>
</table>
