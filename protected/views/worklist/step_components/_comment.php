<div class="step-comments">
    <?php if (!$partial) { ?>
    <div class="flex js-comments-edit" style="<?= $model->comment ? 'display: none;' : '' ?>">
        <div class="cols-11">
            <input class="cols-full js-step-comments" type="text" maxlength="80" placeholder="Comments"
            <?= $model->comment ? 'value="'.$model->comment->comment.'"' : '' ?>/>
    ...
    <div class="flex js-comments-view" style="<?= !$model->comment ? 'display: none;' : '' ?>">
        <div class="cols-11">
            <i class="oe-i comments small pad-right no-click"></i>
                <em class="comment"><?= $model->comment ? $model->comment->comment : '' ?></em>
        </div>
        <?php if (!$partial && in_array((int)$pathway->status, Pathway::inProgressStatuses(), true)) { ?>
