...
class MedicationAttributeAssignment extends BaseActiveRecordVersioned
{
	...
	public function rules()
	{
		...
		return array(
			array('medication_id, medication_attribute_id', 'numerical', 'integerOnly'=>true),
			array('value, description', 'string'),
            array('value', 'required'),
            array('last_modified_user_id, created_user_id', 'length', 'max'=>10),
			array('last_modified_date, created_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, value, description, medication_id, medication_attribute_id, last_modified_user_id, last_modified_date, created_user_id, created_date', 'safe', 'on'=>'search'),
		);
	}
