...
class Patient extends BaseActiveRecord {
	...
	public function relations() {
		return array(
			...
			// Prefer H records for primary address, but fall back to C and then others (T). Unexpired addresses are preferred, but an expired address will be returned if necessary.
			'address' => array(self::HAS_ONE, 'Address', 'parent_id',
				'order' => "FIELD(type,'C','H') DESC, date_end DESC, date_start DESC"
			),
			// Prefer H records for home address, but fall back to C and then others (T). Unexpired addresses are preferred, but an expired address will be returned if necessary.
			'homeAddress' => array(self::HAS_ONE, 'Address', 'parent_id',
				'order' => "FIELD(type,'C','H') DESC, date_end DESC, date_start DESC"
			),
			// Prefer C records for correspond address, but fall back to T and then others (H). Unexpired addresses are preferred, but an expired address will be returned if necessary.
			'correspondAddress' => array(self::HAS_ONE, 'Address', 'parent_id',
				'order' => "FIELD(type,'T','C') DESC, date_end DESC, date_start DESC"
			),
