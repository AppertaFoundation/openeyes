...
=======
    ...
    container = undefined;
		...
		var insert_before = container.next('section');
    while (insert_before.data('elementParentId')==$(new_element).data('elementParentId') && parseInt(insert_before.data('element-display-order')) < parseInt(display_order)) {
			insert_before = insert_before.nextAll('.element').first();
		}
		if (insert_before.length) {
			insert_before.before(new_element);
		} else {
			container.after(new_element);
		}
