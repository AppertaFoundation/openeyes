...
class m120809_093634_create_a_contact_for_all_users extends CDbMigration
{
	public function up()
	{
		...
		foreach ($db->createCommand()->select("*")->from("user")->queryAll() as $user) {
			if (!$uca = $db->createCommand()->select("*")->from("user_contact_assignment")->where("user_id=:user_id",array(':user_id'=>$user['id']))->queryRow()) {
				...
				$contact_id = $db->createCommand()->select("max(id)")->from("contact")->queryScalar();
