...
/**
 * @var Element_OphCiTheatreadmission_CaseNote $element
 */
?>
<?php $name_stub = CHtml::modelName($element) . '[caseNotes]';
// get all the case notes for this element
if ($element->id) {
    $caseNotes = OphCiTheatreadmission_CaseNotes::model()->findAll('element_id = :element_id', array(':element_id' => $element->id));
}
?>
<div class="element-fields">
    <?= CHtml::textField(
        $name_stub . '[case_notes]',
...
<?php if (isset($caseNotes) && is_array($caseNotes)) {
            ...
            <table class="cols-full case-notes">
                ...
                <tbody>
                    ...
                    <?php foreach ($caseNotes as $caseNote) {
                        $text = $caseNote->case_notes;
                        $toolTip = "Created: " . $caseNote->created_date . " by " . $caseNote->createdUser->getFullNameAndTitle();
                        ?>
                        <tr>
                            <td>
                            ...
                            <td>
                                <i class="oe-i remove-circle small pad" data-case-note-id="<?= $caseNote->id ?>">
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
    ...
    $(document).ready(function() {
        ...
        $('.case-notes').on('click', 'tr td .remove-circle', function (e) {
            ...
            $currentDialog = new OpenEyes.UI.Dialog({
                ...
                title: "Are you sure you want to remove this case note?",
        ...
        $(document).on('click', '.oe-popup.case-notes button', function () {
            if ($(this).text() === "YES") {
                $.ajax({
                    'url': baseUrl + '/OphCiTheatreadmission/default/deleteCaseNote',
                    'type': 'POST',
