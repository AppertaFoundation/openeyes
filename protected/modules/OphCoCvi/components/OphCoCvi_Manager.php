...
class OphCoCvi_Manager extends \CComponent
{
     ...
     * @return string
     */
    protected function getDisplayStatus(Element_OphCoCvi_EventInfo $info, ?Element_OphCoCvi_ClinicalInfo $clinical = null)
    {
    ...
    public function getDisplayStatusForEvent(\Event $event)
    {
        ...
        return $this->getDisplayStatus($info, $clinical);
    }
    ...
    {
        return $this->getDisplayStatus($element, $element->clinical_element);
    }
    ...
    {
        if (
            (!array_key_exists('issue_complete', $filter) || (isset($filter['issue_complete']) && (bool)$filter['issue_complete']))
            ...
            (!array_key_exists('issue_incomplete', $filter) || (isset($filter['issue_incomplete']) && (bool)$filter['issue_incomplete']))
            and (isset($filter['show_issued']) && (bool)$filter['show_issued'])
        ) {
