...
class OphCoCorrespondence_API extends BaseAPI
{
    ...
    {
        $macro = LetterMacro::model()->with('firms')->find('name = ? AND firms_firms.firm_id = ?', [$macro_name, $firm->id]);

        if (!$macro) {
            if ($firm->service_subspecialty_assignment_id) {
                $subspecialty_id = $firm->serviceSubspecialtyAssignment->subspecialty_id;

                $macro = LetterMacro::model()->with('subspecialties')->find('subspecialties_subspecialties.subspecialty_id = ? AND name = ?', [$subspecialty_id, $macro_name]);
                if (!$macro) {
                    $macro = LetterMacro::model()->with('sites', 'institutions')->find('(institutions_institutions.institution_id = ? OR sites_sites.site_id = ?) AND name = ?', [Yii::app()->session['selected_institution_id'], $site_id, $macro_name]);
                }
            }
        }
