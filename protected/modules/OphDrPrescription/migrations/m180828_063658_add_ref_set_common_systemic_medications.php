...
class m180828_063658_add_ref_set_common_systemic_medications extends CDbMigration
{
    public function up()
    {
        ...
        $this->execute("INSERT INTO medication_set (`name`, `automatic`) VALUES ('Common systemic medications', 1)");
        ...
        foreach (CommonMedications::model()->findAll() as $med) {
            ...
            $this->execute("INSERT INTO medication_set_auto_rule_medication (medication_set_id, medication_id)
                                VALUES (
                                  $ref_auto_set_id, 
                                  (SELECT id FROM medication WHERE source_old_id = ".$med->medication_id." AND source_subtype = 'medication_drug')
                                )");
    ...
    public function down()
    {
        ...
        $this->execute("DELETE FROM medication_set_item WHERE medication_set_id IN (SELECT id FROM medication_set WHERE `name` = 'Common systemic medications (manual)')");
        $this->execute("DELETE FROM medication_set WHERE `name` = 'Common systemic medications'");
        $this->execute("DELETE FROM medication_set WHERE `name` = 'Common systemic medications (manual)'");
