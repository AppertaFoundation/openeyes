...
class m180828_063658_add_ref_set_common_systemic_medications extends CDbMigration
{
	public function up()
	{
	    ...
	    foreach (CommonMedications::model()->findAll() as $med) {
	        $this->execute("INSERT INTO medication_medication_set (medication_set_id, medication_id)
                                VALUES (
                                  $ref_set_id, 
	...
	public function down()
	{
		...
		$this->execute("DELETE FROM medication_medication_set WHERE medication_set_id IN (SELECT id FROM medication_set WHERE `name` = 'Common systemic medications')");
