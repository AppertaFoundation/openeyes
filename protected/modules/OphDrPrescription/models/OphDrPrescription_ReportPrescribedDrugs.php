...
class OphDrPrescription_ReportPrescribedDrugs extends BaseReport
{
    ...
    public function run()
    {
        $command = Yii::app()->db->createCommand()
            ->select(
                'patient.hos_num, contact.last_name, contact.first_name, patient.dob, address.postcode, d.created_date, drug.tallman, IF(drug.id IN (SELECT drug_id FROM drug_tag WHERE tag_id = 1),1,0) AS preservative_free,
