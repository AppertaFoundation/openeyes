...
class OphDrPrescription_ReportPrescribedDrugs extends BaseReport
{
    ...
    public function toCSV()
    {
        ...
        foreach ($this->items as $item) {
            ...
            $output .= (date('j M Y', strtotime($item['created_date'])) . ' ' . (substr($item['created_date'], 11, 5))) . ', ' . str_replace(',', ';', $drug->tallmanLabel) . ', ';
            $output .= str_replace(',', ';', $item['dose']) . ', ' . $item['frequency'] . ', ' . $item['duration'] . ' ,' . $item['route'] . ' ,' . $item['dispense_condition'] . ' ,' . $item['dispense_location'] . ' ,' . $item['laterality'] . ' ,';
