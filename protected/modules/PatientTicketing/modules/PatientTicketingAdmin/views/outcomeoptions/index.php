...
<div class="row divider cols-9">
    ...
    <?php $this->renderPartial('/outcomeoptions/_filter', [
...
<div class="outcompe-options-wrapper" style="display:<?=($queueset_id ? 'block' : 'none')?>">
    ...
    <div class="cols-7">
        <form id="outcome-optios" method="post">
            <input type="hidden" value="<?= Yii::app()->request->csrfToken ?>" name="YII_CSRF_TOKEN">
            <?=\CHtml::hiddenField('institution_id', $institution->id);?>
            <table id="options" class="standard generic-admin sortable" data-sort-uri="/PatientTicketing/PatientTicketingAdmin/OutcomeOptions/sortOptions">
                <thead>
                <tr>
                    <th>Order</th>
                    <th><input type="checkbox" value="1" name="select-all" id="select-all"></th>
                    <th>Name</th>
                ...
                <?php foreach ($options as $i => $option) : ?>
                    <tr class="" data-row="<?=$i;?>">
                        ...
                        <td>
                            <input type="checkbox" name="selected[]" value="0" id="selected_<?=$i?>">
                        </td>
                        <td>
                            <?=\CHtml::activeTextField($option, "[{$i}]name");?>
                        </td>
                        <td>
                            <?=\CHtml::activeDropDownList($option, "[{$i}]episode_status_id", \CHtml::listData(EpisodeStatus::model()->findAll(), 'id', 'name'), [
                                    'empty' => '-'
                            ]); ?>
                        </td>
                        <td>
                            <?=\CHtml::activeCheckBox($option, "[{$i}]followup");?>
                        </td>
                        <td>
                ...
                <tr>
                    <td colspan="10">
                        ...
                        <input type="hidden" value="/PatientTicketing/admin/ticketAssignOutcomes" name="return_url" id="return_url">
                    </td>
                </tr>
                </tfoot>
            </table>
...
<script>
    ...
    const $form = document.getElementById('outcome-optios');
...
<script id="add_template" type="text/html">
    <tr class="" data-row="{{row}}">
        <td class="reorder">
            <span>↑↓</span>
            <input name="OEModule_PatientTicketing_models_TicketAssignOutcomeOption[{{row}}][id]" id="OEModule_PatientTicketing_models_TicketAssignOutcomeOption_{{row}}_id" type="hidden" value="">
            <input name="OEModule_PatientTicketing_models_TicketAssignOutcomeOption[{{row}}][queueset_id]" id="OEModule_PatientTicketing_models_TicketAssignOutcomeOption_{{row}}_queueset_id" type="hidden" value="{{queueset_id}}">
        </td>
        <td>
            <input type="checkbox" value="0" id="selected_{{row}}">
        </td>
        <td>
            <input name="OEModule_PatientTicketing_models_TicketAssignOutcomeOption[{{row}}][name]" id="OEModule_PatientTicketing_models_TicketAssignOutcomeOption_{{row}}_name" type="text" value="">
        </td>
        <td>
            <?=\CHtml::activeDropDownList(new TicketAssignOutcomeOption(), "[{{row}}]episode_status_id", \CHtml::listData(EpisodeStatus::model()->findAll(), 'id', 'name'), [
                'empty' => '-'
            ]); ?>
        </td>
        <td>
            <?=\CHtml::activeCheckBox(new TicketAssignOutcomeOption(), "[{{row}}]followup");?>
        </td>
        <td>
            <button type="button"><a href="#" class="deleteRow">delete</a></button>
        </td>
