 ...
 */
class OutcomeOptionsController extends BaseAdminController
{
    ...
    public function actionIndex()
    {
        ...
        if (\Yii::app()->request->isPostRequest) {
            ...
            $models = OEModule\PatientTicketing\models\TicketAssignOutcomeOption::model()->findAll($criteria);
        }

        $this->render('/outcomeoptions/index', [
            'options' => $models,
    ...
    {
        $queueset_id = \Yii::app()->request->getParam('queueset_id');
        $data = \Yii::app()->request->getParam('OEModule_PatientTicketing_models_TicketAssignOutcomeOption', []);
        ...
        if (!$this->checkAccess('admin') && ($institution_id != $current_institution->id)) {
            $this->redirect(['/PatientTicketing/PatientTicketingAdmin/OutcomeOptions/index']);
        }
        ...
        $criteria->params[':queueset_id'] = $queueset_id;
        $original_options = OEModule\PatientTicketing\models\TicketAssignOutcomeOption::model()->findAll($criteria);
