...
<fieldset class="element-fields">
    ...
    <?php echo $form->checkBoxes(
        $element, 'AnaestheticType', 'anaesthetic_type', 'Anaesthetic Type',
        false, false, false, false,
        array('fieldset-class' => $element->getError('anaesthetic_type') ? 'highlighted-error' : '')
    ); ?>
    ...
    if (Yii::app()->params['ophtroperationbooking_referral_link']) {
      ...
      <div class="row field-row">
          <?php if ($element->canChangeReferral()) { ?>
            <div class="cols-2">
              <label
                  for="Element_OphTrOperationbooking_Operation_referral_id"><?= $element->getAttributeLabel('referral_id'); ?></label>
            </div>
            <div class="cols-4">
                <?php
                $html_options = array(
            ...
            <div class="cols-2"><label>Referral:</label></div>
            <div class="cols-4">
    ...
    <?php echo $form->datePicker($element, 'decision_date', array('maxDate' => 'today'), array(),
        array_merge($form->layoutColumns, array('field' => 2))) ?>
    <?php $form->radioBoolean($element, 'fast_track') ?>
    <?php $form->radioButtons($element, 'fast_track_discussed_with_patient', array(1 => 'Yes', 0 => 'No'), null,
        false) ?>
    <?php $form->radioBoolean($element, 'special_equipment') ?>
    <?php $form->textArea($element, 'special_equipment_details', array(), true, array(),
        array_merge($form->layoutColumns, array('field' => 4))) ?>
    <?php echo $form->textArea($element, 'comments', array('rows' => 4), false, array(),
        array_merge($form->layoutColumns, array('field' => 4))) ?>
    <?php echo $form->textArea($element, 'comments_rtt', array('rows' => 4), false, array(),
        array_merge($form->layoutColumns, array('field' => 4))) ?>
  <div class="flex-layout">
    <div class="cols-2">
      <label
          for="<?= CHtml::modelName($element) . '[organising_admission_user_id]' ?>"><?= CHtml::encode($element->getAttributeLabel('organising_admission_user_id')) ?>
        :</label>
    </div>
    <div class="cols-10">
      ...
      <span
          class="organising_admission_user"><?php echo $element->organising_admission_user ? $element->organising_admission_user->reversedFullname . ' (<a href="#" class="remove_organising_admission_user">remove</a>)' : 'None' ?></span>
    </div>
  </div>
  <div class="flex-layout">
    <div class="cols-2">
