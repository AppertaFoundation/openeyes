...
class m190424_131625_add_type_to_lab_results_result_timed_numeric extends CDbMigration
{
    public function up()
    {
        $default_result_type = OphInLabResults_Type::model()->find('type = ? ', ['INR']);
        $this->addColumn('et_ophinlabresults_result_timed_numeric',
            'type',
            'int(11) NOT NULL DEFAULT ' . $default_result_type->id);
        $this->addColumn('et_ophinlabresults_result_timed_numeric_version',
            'type',
            'int(11) unsigned NOT NULL DEFAULT ' . $default_result_type->id);
