    ...
    function up() {
        ...
        if(\Yii::app()->db->schema->getTable(self::CONSULTANT_SIGNATURE_TABLE)) {
            ...
            $this->dbConnection->createCommand(
                ...
                ." FROM ".self::CONSULTANT_SIGNATURE_TABLE." t left join user on user.id = signed_by_user_id)"
            )->execute();

            //remove from element_type
            $this->dbConnection->createCommand(
                "DELETE FROM element_type where class_name='".self::CONSULTANT_SIGNATURE_ELEMENT_TYPE."'"
            )->execute();
        }
    }
