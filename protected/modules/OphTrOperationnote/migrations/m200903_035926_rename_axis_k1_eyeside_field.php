    ...
    public function up()
    {
        ...
        $this->execute("CREATE OR REPLACE VIEW et_ophtroperationnote_biometry AS 
            SELECT 
                ...
                (
                    SELECT `oll`.`name`
                    FROM `openeyes`.`ophinbiometry_lenstype_lens` `oll`
                ...
                (
                    ...
                    FROM `openeyes`.`et_ophinbiometry_iol_ref_values` `eoirv`
                    WHERE 
                        (
                ...
                (
                    SELECT `oes`.`name`
                    FROM `openeyes`.`dicom_eye_status` `oes`
                ...
                (
                    SELECT `ocf`.`name`
                    FROM `openeyes`.`ophinbiometry_calculation_formula` `ocf`
            ...
            (
                (
                    (
                        (
                            `openeyes`.`et_ophinbiometry_measurement` `eol` 
                            JOIN `openeyes`.`et_ophinbiometry_calculation` `eoc` ON ((`eoc`.`event_id` = `eol`.`event_id`))
                        ) 
                        LEFT JOIN `openeyes`.`et_ophinbiometry_selection` `eos` ON ((`eos`.`event_id` = `eol`.`event_id`))
                    ) 
                    JOIN `openeyes`.`event` `ev` ON ((`ev`.`id` = `eol`.`event_id`))
                )
                JOIN `openeyes`.`episode` `ep` ON ((`ep`.`id` = `ev`.`episode_id`))
            )
