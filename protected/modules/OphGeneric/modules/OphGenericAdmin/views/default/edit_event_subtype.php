...
<div class="cols-11">
    ...
    <table class="standard cols-full">
        ...
        <tr>
            ...
            <td>
                <?php if ($is_new) {
                    echo \CHtml::activeTextArea($event_subtype, 'display_name');
                } else {
                    echo $event_subtype->display_name;
                } ?>
            </td>
        </tr>
        <tr>
            <td>Dicom Modality Code</td>
            <td><?= \CHtml::activeTextArea($event_subtype, 'dicom_modality_code') ?>
            </td>
        </tr>
        <tr>
            <td>Sub type icon</td>
...
<script type="text/template" id="event-subtype-element-entry">
    <tr>
        <td>
            ...
            <input type="hidden" name="EventSubtype[element_type_entries][{{index}}][element_type_id]"
                   value="{{elementTypeId}}"/>
    ...
    $(document).ready(function () {
        ...
        $('#element_type_id').on('change', function () {
            ...
            if (newElementTypeId) {
                ...
                const new_entry = Mustache.render(template, {
                    index: newIndex,
        ...
        const existing = $('.js-element-type-list input[name$="[element_type_id]"]').map(function () {
            return $(this).val();
        }).get();
