...
<div class="cols-full">
    ...
    $common_operations = CommonPreviousSystemicOperation::model()->findAll(['order' => 'display_order asc']);
    $common_operations_options = CHtml::listData($common_operations, 'id', 'name');
    ?>

    <div id="operations" class="field-row">
        <?php
        $columns = [
            [
                'header' => 'Operation',
            ...
            [
                ...
                'value' => function ($data, $row) use ($gender_options) {
                    echo CHtml::dropDownList("OEModule_OphCiExamination_models_SystemicSurgerySetEntry[$row][gender]", $data->gender, $gender_options, ['empty' => '-- select --']);
                }
            ],
            [
                ...
                'value' => function ($data, $row) {
                    return CHtml::numberField("OEModule_OphCiExamination_models_SystemicSurgerySetEntry[$row][age_min]", $data->age_min, ["style" => "width:55px;"]);
                }
            ],
            [
                ...
                'type' => 'raw',
                'value' => function ($data, $row) {
                    return CHtml::numberField("OEModule_OphCiExamination_models_SystemicSurgerySetEntry[$row][age_max]", $data->age_max, ["style" => "width:55px;"]);
                }
            ],
            [
                'header' => '',
        ...
        $dataProvider = new \CActiveDataProvider(\OEModule\OphCiExamination\models\SystemicSurgerySetEntry::class);
    ...
    <?=\CHtml::button(
        ...
        [
            ...
            'name' => 'cancel',
            'id' => 'et_cancel',
            'data-uri' => '/OphCiExamination/admin/SystemicSurgicalHistoryAssignment/index'
        ]
    ); ?>
</div>

<script type="text/template" id="new_operation_entry" class="hidden">
    <tr data-row="{{row}}">
        <td>
            <div>
                <?php
                $list_data = CHtml::listData(
                    CommonPreviousSystemicOperation::model()->findAll(['order' => 'display_order asc']),
                    'id',
                    'name'
                );
        ...
        <td>
            <?php
                echo CHtml::dropDownList("OEModule_OphCiExamination_models_SystemicSurgerySetEntry[{{row}}][gender]", null, $gender_options, ['empty' => '-- select --']);
            ?>
        </td>
        <td>
            <input style="width:55px;" type="number"
                   name="OEModule_OphCiExamination_models_SystemicSurgerySetEntry[{{row}}][age_min]"
                   id="OEModule_OphCiExamination_models_SystemicSurgerySetEntry_{{row}}_age_min">
        </td>
        <td>
            <input style="width:55px;" type="number"
                   name="OEModule_OphCiExamination_models_SystemicSurgerySetEntry[{{row}}][age_max]"
                   id="OEModule_OphCiExamination_models_SystemicSurgerySetEntry_{{row}}_age_max">
        </td>
        <td>
