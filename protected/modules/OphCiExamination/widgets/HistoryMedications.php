    ...
    protected function updateElementFromData($element, $data)
    {
        ...
        if (array_key_exists('entries', $data)) {
            ...
            foreach ($data['entries'] as $entry_data) {
                ...
                list($start_year, $start_month, $start_day) = array_pad(explode('-', $entry_data['start_date']), 3,0);
                $entry->start_date = \Helper::padFuzzyDate($start_year, $start_month, $start_day);
                list($end_year, $end_month, $end_day) = array_pad(explode('-', $entry_data['end_date']), 3,0);
