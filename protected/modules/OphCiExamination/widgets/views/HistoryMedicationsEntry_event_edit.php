...
<tr data-key="<?=$row_count?>"
    ...
    <td>
        ...
        <input type="hidden" name="<?= $field_prefix ?>[to_be_copied]" class="js-to-be-copied" value="<?php echo (int)$to_be_copied; ?>" />
				<input type="hidden" name="<?= $field_prefix ?>[binded_key]" class="js-binded-key" value="<?= !isset($entry->binded_key) && isset($is_template) && $is_template ? "{{binded_key}}" : $entry->binded_key ?>">
    </td>
