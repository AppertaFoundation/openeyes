...
<div class="row field-row <?php echo $el_model_name ?>_no_treatment_reason_id" id="div_<?php echo $el_model_name . '_' . $side ?>_no_treatment_reason_id"<?php if (!$no_treatment) {
  ...
  <div class="large-8 column end">
      <?php echo $form->dropDownlist($element, $side . '_no_treatment_reason_id', CHtml::listData($no_treatment_reasons, 'id', 'name'), $no_treatment_reasons_opts,
          array('nowrapper' => true)) ?>
  </div>
</div>

<div class="row field-row <?php echo $el_model_name ?>_no_treatment_reason_other"
     id="div_<?php echo $el_model_name . '_' . $side ?>_no_treatment_reason_other"<?php if (!$show_no_treatment_reason_other) {
    ?> style="display: none;"<?php
} ?>>
  <div class="large-3 column">
...
<div id="div_<?php echo $el_model_name . '_' . $side ?>_treatment_fields">
  ...
  <div class="<?php if (!array_key_exists($element->{$side . '_diagnosis1_id'}, $l2_disorders)) {
    ...
    <?php if ($treatments = $element->getInjectionTreatments($side)) {
        ?>
        <?php echo $form->dropDownList($element, $side . '_treatment_id', CHtml::listData($treatments, 'id', 'name'), array('empty' => '- Please select -'), false,
