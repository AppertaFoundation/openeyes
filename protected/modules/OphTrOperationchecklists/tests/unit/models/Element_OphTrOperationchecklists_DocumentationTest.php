...
class Element_OphTrOperationchecklists_DocumentationTest extends ActiveRecordTestCase
{
    /**
     * @var Element_OphTrOperationchecklists_Documentation
     */
    protected Element_OphTrOperationchecklists_Documentation $model;
    ...
    {
        parent::setUp();
        $this->model = new Element_OphTrOperationchecklists_Documentation();
    }

    /**
     * @covers Element_OphTrOperationchecklists_Documentation
     */
    public function testModel()
    {
        $this->assertEquals('Element_OphTrOperationchecklists_Documentation', get_class($this->model), 'Class name should match model');
    }
    ...
    {
        $this->assertEquals('et_ophtroperationchecklists_documentation', $this->model->tableName());
    }

    /**
     * @covers Element_OphTrOperationchecklists_Documentation
     */
    public function testSaveDocumentationData()
    {
        $event = $this->event('event1');

        $element = new Element_OphTrOperationchecklists_Documentation();
        ...
        if (isset($question)) {
            $result = new OphTrOperationchecklists_DocumentationResults();
            ...
            $checklist_results = OphTrOperationchecklists_DocumentationResults::model()->findAll('element_id = :element_id AND question_id = :question_id', array(':element_id' => $element->id, ':question_id' => $question->id));
    ...
    public function tearDown(): void
    {
        foreach ($this->elementIds as $id) {
            foreach (OphTrOperationchecklists_DocumentationResults::model()->findAll('element_id = :element_id', array(':element_id' => $id)) as $t) {
                $t->noVersion()->delete();
            }
            Element_OphTrOperationchecklists_Documentation::model()->noVersion()->deleteByPk($id);
        }
