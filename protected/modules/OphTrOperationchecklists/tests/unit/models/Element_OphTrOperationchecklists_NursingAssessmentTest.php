...
class Element_OphTrOperationchecklists_NursingAssessmentTest extends ActiveRecordTestCase
{
    ...
    public function testSaveNursingData()
    {
        ...
        if (isset($question)) {
            ...
            $checklist_results = OphTrOperationchecklists_NursingResults::model()->findAll('element_id = ? AND question_id = ?', array($element->id, $question->id));
    ...
    public function tearDown()
    {
        foreach ($this->elementIds as $id) {
            foreach (OphTrOperationchecklists_NursingResults::model()->findAll('element_id = ?', array($id)) as $t) {
                $t->noVersion()->delete();
            }
