...
class Element_OphTrOperationchecklists_AdmissionTest extends ActiveRecordTestCase
{
    /**
     * @var Element_OphTrOperationchecklists_Admission
     */
    protected Element_OphTrOperationchecklists_Admission $model;
    ...
    {
        parent::setUp();
        $this->model = new Element_OphTrOperationchecklists_Admission();
    }

    /**
     * @covers Element_OphTrOperationchecklists_Admission
     */
    public function testModel()
    {
        $this->assertEquals('Element_OphTrOperationchecklists_Admission', get_class($this->model), 'Class name should match model');
    }
    ...
    {
        $this->assertEquals('et_ophtroperationchecklists_admission', $this->model->tableName());
    }
    ...
    public function testSaveData()
    {
        ...
        $element = new Element_OphTrOperationchecklists_Admission();
        ...
        if (isset($question)) {
            $result = new OphTrOperationchecklists_AdmissionResults();
            ...
            $checklist_results = OphTrOperationchecklists_AdmissionResults::model()->findAll('element_id = :element_id AND question_id = :question_id', array(':element_id' => $element->id, ':question_id' => $question->id));
    ...
    public function tearDown()
    {
        foreach ($this->elementIds as $id) {
            foreach (OphTrOperationchecklists_AdmissionResults::model()->findAll('element_id = :element_id', array(':element_id' => $id)) as $t) {
                $t->noVersion()->delete();
            }
            Element_OphTrOperationchecklists_Admission::model()->noVersion()->deleteByPk($id);
        }
