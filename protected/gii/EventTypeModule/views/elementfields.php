...
<div class="giiElementContainer" style="margin-bottom: 10px;">
	<div class="giiElement" style="background:#eee;border:1px solid #999;padding:5px;">
		...
		<h4 style="margin-bottom: 0;">
			<select class="elementToAddFieldsTo" name="elementId<?php echo $element_num?>">
				...
				<?php foreach (ElementType::model()->findAll('event_type_id=?',array($event_type_id)) as $element_type) {?>
					<option value="<?php echo $element_type->id?>"<?php if (@$_POST['elementId'.$element_num] == $element_type->id) {?> selected="selected"<?php }?>><?php echo $element_type->name?></option>
				<?php }?>
			</select>
		</h4>
		...
		<input type="submit" class="add_element_field" name="addElementField<?php echo $element_num?>" value="add field"<?php if (!@$_POST['elementId'.$element_num]) {?> style="display: none;"<?php }?> /><br />
	</div>
</div>
