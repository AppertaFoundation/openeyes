			...
			<?php foreach ($filter_fields as $filter_field) { ?>
				<div class="row field-row">
					<div class="large-2 column"><label
							for="<?= $filter_field['field'] ?>"><?= CHtml::encode($model::model()->getAttributeLabel($filter_field['field'])); ?></label>
					</div>
...
<?php foreach ($items as $i => $row) {
	$this->render('_generic_admin_row', array(
		'i' => $i,
		'row' => $row,
...
if (!$get_row && $filters_ready) {
	...
	<div>
		<?php echo EventAction::button('Add', 'admin-add', null, array(
			'class' => 'generic-admin-add small secondary',
			'data-model' => $model,
			'data-new-row-url' => @$this->new_row_url
		))->toHtml() ?>&nbsp;
		<?php echo EventAction::button('Save', 'admin-save', null,
			array('class' => 'generic-admin-save small primary'))->toHtml() ?>&nbsp;
	</div>
	<?= CHtml::endForm() ?>
<?php }
