...
class AdminController extends BaseController
{
	...
	public function actionAddUser()
	{
		...
		if (!empty($_POST)) {
			...
			if (!$user->validate()) {
				...
				Audit::add('admin-users','add',serialize($_POST));
	...
	public function actionEditUser($id)
	{
		...
		if (!empty($_POST)) {
			...
			if (!$user->validate()) {
				...
				Audit::add('admin-users','edit',serialize(array_merge(array('id'=>$id),$_POST)));

				$this->redirect('/admin/users/'.ceil($user->id/$this->items_per_page));
			}
		} else {
			Audit::add('admin-users','view',$id);
		}
	...
	public function actionDeleteUsers() {
		...
		if (!empty($_POST['users'])) {
			foreach (User::model()->findAllByPk($_POST['users']) as $user) {
				...
				if ($result) {
					Audit::add('admin-users','delete',serialize($_POST));
				}
			}
		}
	...
	public function actionAddFirm()
	{
		...
		if (!empty($_POST)) {
			...
			if (!$firm->validate()) {
				...
				Audit::add('admin-firms','add',serialize($_POST));
	...
	public function actionEditFirm($id)
	{
		...
		if (!empty($_POST)) {
			...
			if (!$firm->validate()) {
				...
				Audit::add('admin-firms','edit',serialize(array_merge(array('id'=>$id),$_POST)));
				$this->redirect('/admin/firms/'.ceil($firm->id/$this->items_per_page));
			}
		} else {
			Audit::add('admin-firms','view',$id);
		}
	...
	public function actionContactlabels($id=false)
	{
		...
		Audit::add('admin-contact-labels','list');
	...
	public function searchContacts()
	{
		...
		Audit::add('admin-contacts','search',@$_GET['q']);
	...
	public function actionEditContact()
	{
		...
		if (!empty($_POST)) {
			...
			if (!$contact->validate()) {
				...
				Audit::add('admin-contacts','edit',serialize(array_merge(array('id'=>@$_GET['contact_id']),$_POST)));
				$this->redirect('/admin/contacts?q='.$contact->fullName);
			}
		} else {
			Audit::add('admin-contacts','view',@$_GET['contact_id']);
		}
	...
	public function actionContactLocation()
	{
		...
		Audit::add('admin-contact-locations','view',@$_GET['location_id']);
	...
	public function actionRemoveLocation()
	{
		...
		Audit::add('admin-contact-locations','delete',@$_POST['location_id']);
		...
		if (!empty($_POST)) {
			...
			if (empty($errors)) {
				...
				if (!$cl->save()) {
					...
					Audit::add('admin-contact-locations','add',serialize($_POST));
					$this->redirect(array('/admin/editContact?contact_id='.$contact->id));
				}
			}
		}
	...
	public function actionGetInstitutionSites()
	{
		...
		Audit::add('admin-institution-sites','view',@$_GET['institution_id']);
	...
	public function actionInstitutions($id=false)
	{
		...
		Audit::add('admin-institutions','list');
	...
	public function actionAddInstitution()
	{
		...
		if (!empty($_POST)) {
			...
			if (empty($errors)) {
				...
				Audit::add('admin-institutions','add',serialize($_POST));
	...
	public function actionEditInstitution()
	{
		...
		if (!empty($_POST)) {
			...
			if (empty($errors)) {
				...
				Audit::add('admin-institutions','edit',serialize(array_merge(array('id'=>@$_GET['institution_id']),$_POST)));

				$this->redirect('/admin/institutions');
			}
		} else {
			Audit::add('admin-institutions','view',@$_GET['institution_id']);
		}
	...
	public function actionEditsite()
	{
		...
		if (!empty($_POST)) {
			...
			if (empty($errors)) {
				...
				Audit::add('admin-sites','edit',serialize(array_merge(array('id'=>@$_GET['site_id']),$_POST)));

				$this->redirect('/admin/sites');
			}
		} else {
			Audit::add('admin-sites','view',@$_GET['site_id']);
		}
	...
	public function actionAddContact()
	{
		...
		if (!empty($_POST)) {
			...
			if (!$contact->validate()) {
				...
				Audit::add('admin-contacts','add',serialize($_POST));
	...
	public function actionAddContactLabel()
	{
		...
		if (!empty($_POST)) {
			...
			if (!$contactlabel->validate()) {
				...
				Audit::add('admin-contact-labels','add',serialize($_POST));
	...
	public function actionEditContactLabel($id)
	{
		...
		if (!empty($_POST)) {
			...
			if (!$contactlabel->validate()) {
				...
				Audit::add('admin-contact-labels','edit',serialize(array_merge(array('id'=>$id),$_POST)));

				$this->redirect('/admin/contactlabels/'.ceil($contactlabel->id/$this->items_per_page));
			}
		} else {
			Audit::add('admin-contact-labels','view',$id);
		}
	...
	public function actionDeleteContactLabel()
	{
		...
		if ($count == 0) {
			...
			Audit::add('admin-contact-labels','delete',@$_POST['contact_label_id']);
		}
	...
	{
		Audit::add('admin-data-sources','list');
	...
	public function actionEditDataSource($id)
	{
		...
		if (!empty($_POST)) {
			...
			if (!$source->validate()) {
				...
				Audit::add('admin-data-sources','edit',serialize(array_merge(array('id'=>$id),$_POST)));
				$this->redirect('/admin/datasources/'.ceil($source->id/$this->items_per_page));
			}
		} else {
			Audit::add('admin-data-sources','view',$id);
		}
	...
	public function actionAddDataSource()
	{
		...
		if (!empty($_POST)) {
			...
			if (!$source->validate()) {
				...
				Audit::add('admin-data-sources','add',serialize($_POST));
	...
	public function actionDeleteDataSources()
	{
		if (!empty($_POST['source'])) {
			...
			Audit::add('admin-data-sources','delete',serialize($_POST['source']));
		}
			...
			foreach (Firm::model()->findAllByPk($_POST['firms']) as $firm) {
				try {
					...
					if (!$firm->delete()) {
						...
						Audit::add('admin-firms','delete',$firm_id);
					}
				} catch (Exception $e) {
					$result = 0;
				}
	...
	{
		Audit::add('admin-commissioning-bodies','list');
	...
	public function actionEditCommissioningBody()
	{
		...
		if (!empty($_POST)) {
			...
			if (empty($errors)) {
				...
				Audit::add('admin-commissioning-bodies',$method,serialize($audit));

				$this->redirect('/admin/commissioning_bodies');
			}
		} else {
			Audit::add('admin-commissioning-bodies','view',@$_GET['commissioning_body_id']);
		}
	...
	public function actionDeleteCommissioningBodies()
	{
		...
		if (CommissioningBody::model()->deleteAll($criteria)) {
			echo "1";
			Audit::add('admin-commissioning-bodies','delete',serialize($_POST));
		} else {
			echo "0";
		}
	}
	...
	{
		Audit::add('admin-commissioning-body-types','list');
	...
	public function actionEditCommissioningBodyType()
	{
		...
		if (!empty($_POST)) {
			...
			if (empty($errors)) {
				...
				Audit::add('admin-commissioning-body-types',$method,serialize($audit));
	...
	public function actionDeleteCommissioningBodyTypes()
	{
		...
		Audit::add('admin-commissioning-body-types','delete',serialize($_POST));
	...
	{
		Audit::add('admin-commissioning-body-services','list');
	...
	public function actionEditCommissioningBodyService()
	{
		...
		if (!empty($_POST)) {
			...
			if (empty($errors)) {
				...
				Audit::add('admin-commissioning-body-services',$method,serialize($audit));
	...
	public function actionDeleteCommissioningBodyServices()
	{
		...
		Audit::add('admin-commissioning-body-services','delete',serialize($_POST));
	...
	public function actionEditCommissioningBodyServiceType()
	{
		...
		if (!empty($_POST)) {
			...
			if (empty($errors)) {
				...
				Audit::add('admin-commissioning-body-service-types',$method,serialize($audit));
	...
	public function actionDeleteCommissioningBodyServiceTypes()
	{
		...
		Audit::add('admin-commissioning-body-service-types','delete',serialize($_POST));
