...
mkdir -p docker
wget https://raw.githubusercontent.com/openeyes/Sample/master/sql/openeyes_sample_data.sql -N -O $PWD/docker/data.sql

docker run --name $1 -v $PWD/docker:/docker-entrypoint-initdb.d -e MYSQL_ROOT_PASSWORD=rootpass -e MYSQL_DATABASE=openeyes -e MYSQL_USER=openeyes -e MYSQL_PASSWORD=oe_test -p 3306 -d mysql
